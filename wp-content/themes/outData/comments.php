...
$discussion = outdata_get_discussion_data();
?>
...
<div id="comments" class="<?php echo comments_open() ? 'comments-area' : 'comments-area comments-closed'; ?>">
	<div class="<?php echo $discussion->responses > 0 ? 'comments-title-wrap' : 'comments-title-wrap no-responses'; ?>">
		...
		if ( comments_open() ) {
			if ( have_comments() ) {
				_e( 'Join the Conversation', 'outdata' );
			} else {
				_e( 'Leave a comment', 'outdata' );
			}
		} else {
			if ( '1' == $discussion->responses ) {
				/* translators: %s: post title */
				printf( _x( 'One reply on &ldquo;%s&rdquo;', 'comments title', 'outdata' ), get_the_title() );
			} else {
				printf(
		...
		if ( have_comments() && comments_open() ) {
			get_template_part( 'template-parts/post/discussion');
		}
		?>
	...
	if ( have_comments() ) :
		...
		<ol class="comment-list">
			...
			wp_list_comments(
				array(
					'avatar_size' => outdata_get_avatar_size(),
					'short_ping'  => true,
					'style'       => 'ol',
				)
			);
		...
		if ( ! comments_open() ) :
			?>
			<p class="no-comments">
				<?php _e( 'Comments are closed.', 'outdata' ); ?>
			</p>
			<?php
		endif;
